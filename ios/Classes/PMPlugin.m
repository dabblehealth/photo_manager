...
- (void)checkAuthStatus:(void (^)(PHAuthorizationStatus status))handler {
...
- (void)handleMethodResult:(FlutterMethodCall *)call handler:(ResultHandler *)handler manager:(PMManager *)manager notificationManager:(PMNotificationManager *)notificationManager {
    if ([call.method isEqualToString:@"getAssetPathList"]) {
        ...
        [manager getFullSizeFileWithId:id
                         ...
                         resultHandler:handler
                       progressHandler:progressHandler];
    } else if ([call.method isEqualToString:@"checkPermissionStatus"]) {
        [self checkAuthStatus:^(PHAuthorizationStatus status) {
                  BOOL showPopup = status == PHAuthorizationStatusLimited;
                  [handler reply:@(showPopup)];
                }];
